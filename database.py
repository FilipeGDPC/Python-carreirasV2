...
def carrega_vaga_db(id):
  with engine.connect() as conn:
    resultado = conn.execute(text("SELECT * FROM vagas WHERE id = :val"), {"val": id})
...
def adiciona_inscricao(id_vaga, dados):
  with engine.connect() as conn:
    query = text(
      "INSERT INTO inscricoes(vaga_id, nome, email, linkedin, experiencia) VALUES(:vaga_id, :nome, :email, :linkedin, :experiencia)"
    )
    conn.execute(
      query, {
        'vaga_id': id_vaga,
